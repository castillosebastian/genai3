...
class AISearch:
    ...
    @sk_function(
        description="This function answer all questions from the user related to finance documents",
        name="index_search",
        input_description="The query may be any general question that may be found in Documents (context).",
    )
    async def search(self, query="Revenue of Microsoft", k_nearest_neighbors=5, top=5, embed_field='Embedding', select_fields=None, query_type="simple", filter=None):
...
#Modify the main code to handle async
async def main():
    ai_search = AISearch()

    hybrid_search_results = await ai_search.search(query="Revenue of Microsoft", query_type="hybrid", top=3)
